...
class OdometryMotionModel:
  ...
  '''
    Propagate the particles forward (in-place) by the estimated motion and add sampled gaussian noise
      ...
      control: The estimated chage in the car's pose [x_noise, y_noise, theta_noise]
  '''
...
class KinematicMotionModel:
  ...
  def apply_motion_model(self, proposal_dist, control):
    ...
    if delta_magnitude < 1e-2:
        x_noise = v * np.cos(proposal_dist[:, 2]) * dt
        y_noise = v * np.sin(proposal_dist[:, 2]) * dt
        theta_noise = 0
    else:
        beta = np.arctan(0.5 * np.tan(delta))
        sin2beta = np.sin(2 * beta)
        theta_noise = ((v / self.CAR_LENGTH) * sin2beta) * dt
        x_noise = (self.CAR_LENGTH/sin2beta)*(np.sin(proposal_dist[:,2]+theta_noise)-np.sin(proposal_dist[:,2]))        
        y_noise = (self.CAR_LENGTH/sin2beta)*(-1*np.cos(proposal_dist[:,2]+theta_noise)+np.cos(proposal_dist[:,2]))
 
    # Propagate particles forward, and add sampled model noise
    proposal_dist[:, 0] += x_noise + np.random.normal(loc=0.0, scale=KM_X_FIX_NOISE+KM_X_SCALE_NOISE*v_mag, size=proposal_dist.shape[0])
    proposal_dist[:, 1] += y_noise + np.random.normal(loc=0.0, scale=KM_Y_FIX_NOISE+KM_Y_SCALE_NOISE*v_mag, size=proposal_dist.shape[0])
    proposal_dist[:, 2] += theta_noise + np.random.normal(loc=0.0, scale=KM_THETA_FIX_NOISE, size=proposal_dist.shape[0])
