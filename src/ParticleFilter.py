...
class ParticleFilter():
  ...
  def initialize_global(self):
    ...
    in_bounds = np.zeros((self.particles.shape[0],3))
    
    # Loop through permissible states, each iteration drawing particles with
    # different rotation
    for i in xrange(angle_step):
      in_bounds[i*(self.particles.shape[0]/angle_step):(i+1)*(self.particles.shape[0]/angle_step),0] = permissible_y[indices]
      in_bounds[i*(self.particles.shape[0]/angle_step):(i+1)*(self.particles.shape[0]/angle_step),1] = permissible_x[indices]
      in_bounds[i*(self.particles.shape[0]/angle_step):(i+1)*(self.particles.shape[0]/angle_step),2] = i*(2*np.pi / angle_step) 
     
    # Transform permissible states to be w.r.t world 
    Utils.map_to_world(in_bounds, self.map_info)
    
    # Reset particles and weights
    self.particles[:,:] = in_bounds[:,:]
  ...
  '''
    ...
    Uses weighted cosine and sine averaging to more accurately compute average theta
      https://en.wikipedia.org/wiki/Mean_of_circular_quantitiesf
  '''
