...
class ReSampler:
  ...
  def resample_naiive(self):
    ...
    random_index = np.random.choice(n_particles, n_particles, replace=True)
    m_particles = np.zeros((n_particles,3))
    m_weights = np.zeros(n_particles)
    
    print self.weights
    for i in xrange(n_particles):
        chosen_bin = random_index[i]
  ...
  def resample_low_variance(self):
    ...
    new_array = np.zeros((n_particles,3))
    m_weights = np.zeros(n_particles)
    ...
    for m in xrange (0, n_particles):
      ...
      new_array[m,:] = self.particles[i,:]

    self.particles[:,:] = new_array[:,:]
    self.weights[:] = 1.0/n_particles
